...
```{bash}
plink --file RISG.raw --chr-set 94 --allow-extra-chr --remove FailedQC.remove --recode --out RISG.passQC
```
...
```{bash}
for pop in {FI,MV,CT,NR,NEH,GH}; do plink --file $pop.uf --chr-set 94 --allow-extra-chr --geno 0.05 --chr 1-10 --mind 0.05 --hwe 1e-6 --recode --out $pop.f; done
```
...
Next our plink files were filtered to only include common SNPs using the text file created earlier and kept SNPs with a minor allele frequency greater than 0.05 in all populations.

```{bash}
plink --file RISG.all.uf --allow-extra-chr --chr-set 94 --extract SNPs.txt --maf 0.05 --recode --out RISG.all.f
plink --file RISG.wild.uf --allow-extra-chr --chr-set 94 --extract SNPs.txt--maf 0.05 --recode --out RISG.wild.f
```
