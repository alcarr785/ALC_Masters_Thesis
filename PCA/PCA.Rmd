...
Making tables for principal components and percent of variance explained
```{r}
# set names
...
pca <- pca %>% mutate(Population = recode(Population, FI = 'CL1', NEH = 'CL2')) %>%
  ...
  mutate(Population = factor(Population, levels = c("CT","GH","MV","NR","CL1","CL2")))
...
PCA_all <- ggplot(pca, aes(PC1, PC2, col = Population)) + 
  ...
  theme(
    ...
    xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))+ ggtitle("PC1 vs. PC2 All Populations")
plot(PCA_all)
```

Making tables for principal components and percent of variance explained for wild populations
```{r}
# set names
...
Plotting PCA for wild populations
```{r}
PCA_wild <- ggplot(pca_wild, aes(PC1, PC2, col = Population)) + 
  ...
  theme(
    ...
    axis.line.y = element_line(size = 1, linetype = "solid", colour = "black"))+ 
  scale_color_brewer(palette="Set2")+
  xlab(paste0("PC1 (", signif(pve_wild$pve_wild[1], 3), "%)"))+
  ylab(paste0("PC2 (", signif(pve_wild$pve_wild[2], 3), "%)"))+ ggtitle("PC1 vs. PC2 Wild Populations")
plot(PCA_wild)
```

```{r}
library(cowplot)
plot_grid(PCA_all, PCA_wild, labels = c("A","B"))
```
